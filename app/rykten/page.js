...
async function getData() {
  ...
  const [ryktenRes, spridningarRes] = await Promise.all([
    fetch(`${SB_URL}/rest/v1/rykten?order=antal_spridningar.desc&limit=100&select=id,innehall,om_agent,ursprung_agent,sanning,kanda_av,antal_spridningar,parent_rykte_id,skapad`, { headers: h, next: { revalidate: 900 } }),
    fetch(`${SB_URL}/rest/v1/rykte_spridningar?order=skapad.desc&limit=200&select=rykte_id,fran_agent,till_agent,kanal,skapad`, { headers: h, next: { revalidate: 900 } }),
  ]);
  return {
