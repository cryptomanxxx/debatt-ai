...
export default async function ChattDebattPage({ params }) {
  ...
  const navLink = (href, lbl, active) => (
    <a href={href} style={{ flex: 1, textAlign: "center", background: active ? "#f8fafc15" : "transparent", border: `1px solid ${active ? "#f8fafc50" : C.border}`, color: active ? "#f8fafc" : C.textMuted, padding: "6px 14px", borderRadius: "4px", fontSize: "13px", letterSpacing: "0.05em", fontFamily: "Georgia, serif", textDecoration: "none" }}>{lbl}</a>
  );
