...
async function getData() {
  const [stRes, vaRes, prRes, ldRes] = await Promise.all([
    fetch(`${SB_URL}/rest/v1/handel_städer?order=id.asc`,        { headers: H, next: { revalidate: 900 } }).catch(() => null),
    fetch(`${SB_URL}/rest/v1/handel_varor?order=id.asc`,         { headers: H, next: { revalidate: 900 } }).catch(() => null),
    fetch(`${SB_URL}/rest/v1/handel_priser?select=*`,            { headers: H, next: { revalidate: 900 } }).catch(() => null),
    fetch(`${SB_URL}/rest/v1/handel_spelare?order=mynt.desc&limit=15&select=id,smeknamn,mynt,stad_id,spelare_typ`,
                                                                  { headers: H, next: { revalidate: 900 } }).catch(() => null),
  ]);
  return {
