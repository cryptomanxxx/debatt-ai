...
const VARA_IKON = { el: "⚡", spannmål: "🌾", maskiner: "🏭", malm: "⛏️", tjänster: "🏙️", fisk: "🌊", virke: "🌲" };
...
export default function VarumarknadVy({ resurspriser, auktioner, handelLog, lager }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
      ...
      {/* ── RÅVARUPRISER ── */}
      <section>
        <Label>Råvarupriser · {resurspriser.length} typer</Label>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: "10px" }}>
          {VAROR.map(vara => {
            const typ = Object.entries(TYP_TILL_VARA).find(([, v]) => v === vara)?.[0];
            const res = resursMap[typ] || {};
            const mult = parseFloat(res.pris_multiplier || 1);
            const pris = Math.round(BASPRIS[vara] * mult);
