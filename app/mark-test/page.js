...
async function fetchData() {
  ...
  const [zonerRes, agareRes] = await Promise.allSettled([
    fetch(`${SB}/rest/v1/mark_zoner?select=id,namn,typ,hex_col,hex_row,koppris,veckoinkomst`, { headers: h, next: { revalidate: 900 } }),
    fetch(`${SB}/rest/v1/mark_agare?select=zon_id,agent`, { headers: h, next: { revalidate: 900 } }),
  ]);
