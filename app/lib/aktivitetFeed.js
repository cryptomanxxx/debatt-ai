/**
 * Delad byggare för "Senaste aktivitet"-händelser — hämtar från ~30 olika
 * Supabase-tabeller parallellt och bygger en enhetlig, tidssorterad lista av
 * {typ, ikon, text, href, skapad, farg}-objekt.
 *
 * Utbruten ur app/api/aktivitet/route.js (✅106/✅107) så att både
 * startsidans widget (topp-10 med garanterade artikelplatser, 25s cache)
 * OCH det fullständiga arkivet på /aktivitet (paginerat, större per-källa-
 * urval) delar exakt samma händelsedefinitioner istället för att de
 * dupliceras och kan glida isär.
 *
 * `limit` styr hur många rader som hämtas PER KÄLLA (samma tal för alla ~30
 * tabeller — sourcerna har små sinsemellan olika standardvärden i den
 * ursprungliga koden, men eftersom bara toppen av den kombinerade,
 * tidssorterade listan någonsin används spelar det ingen roll för
 * korrektheten om en källa råkar få några fler kandidater än den behöver).
 */

const SB_URL = "https://fmwxftnistkoqazfwnuj.supabase.co";
const SB_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

// Delade konstanter för /aktivitet-arkivet (✅107) — måste vara identiska i
// SSR-sidan (app/aktivitet/page.js) och paginerings-API:et
// (app/api/aktivitet/arkiv/route.js), annars kan sidgränsen glida (dubblett-
// eller luckrisk vid "Ladda fler"). Källan till sanning ligger här, inte
// duplicerad i de två filerna.
