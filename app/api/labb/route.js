  ...
  if (groqKey && providerReady("groq")) {
    ...
    try {
      const r = await fetch(GROQ_URL, { method: "POST", headers: { Authorization: `Bearer ${groqKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({ model: "llama-3.3-70b-specdec", messages: msgs, max_tokens: 250, temperature: 1.0 }),
        signal: AbortSignal.timeout(6000) });
      if (r.ok) {
        const json = await r.json();
        const text = json.choices?.[0]?.message?.content?.trim() ?? "";
        if (text) { logAiCall({ provider: "groq", model: "llama-3.3-70b-specdec", source: "labb", status: "ok", latency_ms: Date.now() - t0 }); await logLabb({ amne: amne.trim(), aggressivitet: Number(aggressivitet)||50, faktafokus: Number(faktafokus)||50, humor: Number(humor)||50, optimism: Number(optimism)||50, provider: "groq" }); return Response.json({ svar: text }); }
      }
