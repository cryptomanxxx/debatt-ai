  ...
  if (groqKey && providerReady("groq")) {
    ...
    try {
      const r = await fetch(GROQ_URL, {
        ...
        body: JSON.stringify({ model: "llama-3.3-70b-specdec", messages: msgs, max_tokens: 200, temperature: 0.9 }),
      ...
      if (r.ok) {
        ...
        if (text) {
          logAiCall({ provider: "groq", model: "llama-3.3-70b-specdec", source: "agent-utmaning", status: "ok", latency_ms: Date.now() - t0 });
