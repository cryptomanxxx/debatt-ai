  ...
  if (groqKey && providerReady("groq_kanal")) {
    ...
    try {
      const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: "openai/gpt-oss-120b", messages: msgs, max_tokens: 350, temperature: 0.4 }),
      ...
      if (r.ok) {
        ...
        const text = json.choices[0].message.content.trim();
        if (text && text !== rubrik) {
          logAiCall({ provider: "groq", model: "openai/gpt-oss-120b", source: "kanal", status: "ok", latency_ms, input_tokens: json?.usage?.prompt_tokens, output_tokens: json?.usage?.completion_tokens });
          return Response.json({ text }, { headers: { "X-Provider": "groq" } });
        }
      } else {
        if (r.status === 429) markProviderDown("groq_kanal");
        logAiCall({ provider: "groq", model: "openai/gpt-oss-120b", source: "kanal", status: r.status === 429 ? "rate_limited" : "error", latency_ms });
      }
    } catch {
      logAiCall({ provider: "groq", model: "openai/gpt-oss-120b", source: "kanal", status: "timeout", latency_ms: Date.now() - t0 });
    }
  }
