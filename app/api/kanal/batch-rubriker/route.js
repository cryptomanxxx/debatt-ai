  ...
  if (cbKey && providerReady("cerebras")) {
    ...
    try {
      const r = await fetch("https://api.cerebras.ai/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: "llama3.3-70b", messages: msgs, max_tokens: 600, temperature: 0.1 }),
      ...
      if (r.ok) {
        ...
        const parsed = parseNumberedList(text, rubriker.length);
        if (parsed) {
          logAiCall({ provider: "cerebras", model: "llama3.3-70b", source: "kanal-batch-en", status: "ok", latency_ms: Date.now() - t0, input_tokens: json.usage?.prompt_tokens ?? null, output_tokens: json.usage?.completion_tokens ?? null });
          return Response.json({ translated: parsed });
        }
        logAiCall({ provider: "cerebras", model: "llama3.3-70b", source: "kanal-batch-en", status: "parse_fail", latency_ms: Date.now() - t0 });
      } else {
        if (r.status === 429) markProviderDown("cerebras");
        logAiCall({ provider: "cerebras", model: "llama3.3-70b", source: "kanal-batch-en", status: `error_${r.status}`, latency_ms: Date.now() - t0 });
      }
    } catch {
      logAiCall({ provider: "cerebras", model: "llama3.3-70b", source: "kanal-batch-en", status: "timeout", latency_ms: Date.now() - t0 });
    }
  }
