  ...
  if (cbKey && providerReady("cerebras")) {
    ...
    try {
      const r = await fetch("https://api.cerebras.ai/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: "llama-3.3-70b", messages: msgs, max_tokens: 1200, temperature: 0.3 }),
        signal: AbortSignal.timeout(12000),
      });
      if (r.ok) {
        const json = await r.json();
        ...
        const parsed = parseNumberedList(text, items.length);
        if (parsed) {
          logAiCall({ provider: "cerebras", model: "llama-3.3-70b", source: "kanal-batch", status: "ok", latency_ms: Date.now() - t0, input_tokens: json.usage?.prompt_tokens ?? null, output_tokens: json.usage?.completion_tokens ?? null });
          return Response.json({ expanded: parsed });
        }
        logAiCall({ provider: "cerebras", model: "llama-3.3-70b", source: "kanal-batch", status: "parse_fail", latency_ms: Date.now() - t0 });
      } else {
        if (r.status === 429) markProviderDown("cerebras");
        logAiCall({ provider: "cerebras", model: "llama-3.3-70b", source: "kanal-batch", status: `error_${r.status}`, latency_ms: Date.now() - t0 });
      }
    } catch {
      logAiCall({ provider: "cerebras", model: "llama-3.3-70b", source: "kanal-batch", status: "timeout", latency_ms: Date.now() - t0 });
    }
  }
