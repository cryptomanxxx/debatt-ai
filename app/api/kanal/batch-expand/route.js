  ...
  if (groqKey && providerReady("groq_kanal")) {
    ...
    try {
      const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: "openai/gpt-oss-120b", messages: msgs, max_tokens: 1200, temperature: 0.3 }),
      ...
      if (r.ok) {
        ...
        const parsed = parseNumberedList(text, items.length);
        if (parsed) {
          logAiCall({ provider: "groq", model: "openai/gpt-oss-120b", source: "kanal-batch", status: "ok", latency_ms: Date.now() - t0, input_tokens: json.usage?.prompt_tokens ?? null, output_tokens: json.usage?.completion_tokens ?? null });
          return Response.json({ expanded: parsed });
        }
        logAiCall({ provider: "groq", model: "openai/gpt-oss-120b", source: "kanal-batch", status: "parse_fail", latency_ms: Date.now() - t0 });
      } else {
        if (r.status === 429) markProviderDown("groq_kanal");
        logAiCall({ provider: "groq", model: "openai/gpt-oss-120b", source: "kanal-batch", status: `error_${r.status}`, latency_ms: Date.now() - t0 });
      }
    } catch {
      logAiCall({ provider: "groq", model: "openai/gpt-oss-120b", source: "kanal-batch", status: "timeout", latency_ms: Date.now() - t0 });
    }
  }
