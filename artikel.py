...
def skriv_artikel_om_nyhet(agent: dict, nyhet: dict, extra_kontext: str = "", fmt: dict | None = None, buffs: dict | None = None, status: dict | None = None, koalitions_kontext: str = "", kris_kontext: str = "", minne_kontext: str = "", ki_kontext: str = "", strategi_kontext: str = "", mark_kontext: str = "", foretag_kontext: str = "", hist_kontext: str = "") -> str:
    ...
    user_msg = (
        ...
        "till det nyheten faktiskt rapporterar. Om rubriken/ingressen bara ger "
        "dig lite att gå på: håll dig till det som faktiskt står där och "
        "fördjupa med relevans, sammanhang och konsekvenser i allmänna termer "
        "— hitta ALDRIG på egna konkreta händelsedetaljer (namn, platser, "
        "specifika skeenden) som inte nämns i källan ovan, bara för att fylla "
        "ut ordkravet. Om rubriken eller ingressen är på engelska ska du ändå "
        "skriva hela artikeln på svenska.\n\n"
        ...
        "- Formatstilen nedan beskriver HUR du normalt bygger upp en artikel "
        "i det formatet — men här gäller den ENDAST för hur du väver in ditt "
        "eget, korta perspektiv (t.ex. i en avslutande passage). Om "
        "formatstilen säger att du ska INLEDA artikeln med en tes, en "
        "förutsägelse, ett avvisande av en rådande uppfattning eller ett "
        "läsarproblem — ignorera det helt. Artikeln ska alltid INLEDAS med "
        "den faktabaserade beskrivningen av nyheten, oavsett vad formatstilen "
        "annars säger om hur man öppnar en text:\n"
        f"{fmt['instruktion']}\n"
